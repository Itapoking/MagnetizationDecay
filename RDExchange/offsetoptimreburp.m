function Mfull = offsetoptimreburp(M0, M00,  T1, T2, T, dw, time, npoints, Bampx,Bampy, tpulse, k)
    ndw = size(dw, 1);
    Mfull = zeros(npoints, 3*2*ndw+1);
    Npul = size(Bampx, 1);
    dt = tpulse(1); Npoints = size(Bampx,1);
    Minit = M0;
    ...
    for i=1:ndw
        ...
        for j=1:Npoints
            [ttemp, Mtrajtemp] = ode113(@(t, M) twodensBMConnell(M, dwoff, T1, T2, T, Bampx(j), Bampy(j), M00, k), linspace(0, dt, 3), M0);
            % Mtrajtemp
            M0 = Mtrajtemp(3,:)';
        end
