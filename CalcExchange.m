...
M0a = [0, 3/360*2*pi, 0.038, 10];  % theta, phi, |M0|, enhancement
M0b = [0, 0/360*2*pi, 0.038, 10];
...
[Minit, K, mask, grid, dw] = Initialize_box(gridsize, M0a, M0b, shapesize, DFTset, offset, length, height, shim);
dt = tpul(1);
Nstep = 1e3;
...
for n = 1:Nstep
    ...
    [t, M] = ode113(@(t, M) BlochMcConnell(M, dw, zeros(size(wdip)), wrd, R1, R2, M0a(end-1), M0b(end-1), gridsize(1), gridsize(2), gridsize(3), mask, k), linspace(0, dt, 3), Minit);
    Minit = M(3, :);
    [Mxa, Mya, Mza, Mnorma] = M_averaging(Minit(1:3*nstepsmal), mask);
    [Mxb, Myb, Mzb, Mnormb] = M_averaging(Minit(3*nstepsmal + 1:6*nstepsmal), mask);
    % [wdipx, wdipy, wdipz, wdipnorm] = M_averaging(wdip, mask);
    % [wrdx, wrdy, wrdz, wrdnorm] = M_averaging(wrd, mask);
    observables(n, 1) = dt*(n-1);
    observables(n, 2) = Mxa; observables(n, 3) = Mya; observables(n, 4) = Mza; observables(n, 5) = Mnorma;
end
